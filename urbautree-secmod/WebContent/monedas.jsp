...
<html lang="en">
	...
	<%		
		...
		ArrayList<MonedaBean> list = main.get( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((MonedaBean)list.get( 0 )).getTotal_regs();
		}
	...
	<script>
		function edit( id ){
			location.replace( "monedas-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "monedas-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "monedas-detail.jsp?mode=view&id="+id);
		}
		function add(){
			location.replace( "monedas-detail.jsp?mode=add" );
		}
	</script>
	</head>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          ...
          <div class="col-lg-6">
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
						  ...
						  <% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_MONEDAS, Constants.OPTIONS_ADD)){ %>		
                          ...
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> MONEDAS</h4>
	                  	  	  <hr>
	                  	  	  <thead>
                              <tr>
                                  <th class="hidden-phone">Nombre</th>                                                                    
                                  <th class="hidden-phone">Simbolo</th>
                                  <th></th>
                              </tr>
                              </thead>
                              <tbody>
                              ...
                              <tr>
							  	...
							  	<td><%= bean.getSimbolo() %></td>
								<td>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_MONEDAS, Constants.OPTIONS_MODIFY)){ %>
										<button class="btn btn-primary btn-xs" onclick="edit('<%= bean.getId() %>');"><i class="fa fa-pencil"></i></button>
									<%}%>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_MONEDAS, Constants.OPTIONS_DELETE)){ %>
										<button class="btn btn-danger btn-xs" onclick="removereg('<%= bean.getId() %>');"><i class="fa fa-trash-o "></i></button>
									<%}%>	
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_MONEDAS, Constants.OPTIONS_VIEW)){ %>
		...
		%>
					  ...
					  <% if( backButton ) {%>
					  <li class="previous">
					    		<a href="monedas.jsp?q=<%= request.getParameter("q") %>&from=<%= from - Constants.ITEMS_PER_PAGE  %>">
					  ...
					  <% } %>
					    <% if( forwardButton ){  %>
					    <li class="next">
					    	<a href="monedas.jsp?q=<%= request.getParameter("q") %>&from=<%= end  %>">
