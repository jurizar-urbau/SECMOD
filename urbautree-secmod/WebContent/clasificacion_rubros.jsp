...
<html lang="en">
	...
	ClasificacionRubrosMain main = new ClasificacionRubrosMain();
		...
		ArrayList<ClasificacionRubroBean> list = main.get( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((ClasificacionRubroBean)list.get( 0 )).getTotal_regs();
		}
	%>
	<script>
		function edit( id ){
			location.replace( "clasificacion_rubros-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "clasificacion_rubros-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "clasificacion_rubros-detail.jsp?mode=view&id="+id);
		}
		function add(){
			location.replace( "clasificacion_rubros-detail.jsp?mode=add" );
		}
	</script>
	</head>
          ...
          <div class="col-lg-6">
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
          				<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_CLASIFICACIONRUBROS, Constants.OPTIONS_ADD)){ %>
          				  <span class="pull-right">
          				  	<button type="button" class="btn btn-success" onclick="add();">+</button>&nbsp;&nbsp;&nbsp;          				  
          				  </span>
          				<%} %>  
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> CLASIFICACION DE RUBROS</h4>
	                  	  	  <hr>
	                  	  	  <thead>
                              <tr>
                              ...
                              <tbody>
                              <%
                              	for( ClasificacionRubroBean bean : list ){
                              %>
                              <tr>
							  	<td><%= bean.getDescription() %></td>
								<td>
								<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_CLASIFICACIONRUBROS, Constants.OPTIONS_MODIFY)){ %>
									<button class="btn btn-primary btn-xs" onclick="edit('<%= bean.getId() %>');"><i class="fa fa-pencil"></i></button>
								<%}%>	
								<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_CLASIFICACIONRUBROS, Constants.OPTIONS_DELETE)){ %>
