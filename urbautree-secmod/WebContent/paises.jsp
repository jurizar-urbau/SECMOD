...
<html lang="en">
	...
	<%		
		...
		ArrayList<PaisBean> list = main.get( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((PaisBean)list.get( 0 )).getTotal_regs();
		}
	...
	<script>
		function edit( id ){
			location.replace( "paises-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "paises-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "paises-detail.jsp?mode=view&id="+id);
		}
		function add(){
			location.replace( "paises-detail.jsp?mode=add" );
		}
	</script>
	</head>
          ...
          <div class="col-lg-6">
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
						  ...
						  <% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PAISES, Constants.OPTIONS_ADD)){ %>		
                          ...
                          <table class="table table-striped table-advance table-hover">
                              ...
                              <tbody>
                              <%
                              	for( PaisBean bean : list ){
                              %>
                              <tr>
							  	<td><%= bean.getNombre() %></td>
							  	<td><%= bean.getMonedaNombre() %></td>
							  	<td><%= bean.getMonedaSimbolo() %></td>
								<td>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PAISES, Constants.OPTIONS_MODIFY)){ %>
										<button class="btn btn-primary btn-xs" onclick="edit('<%= bean.getId() %>');"><i class="fa fa-pencil"></i></button>
									<%}%>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PAISES, Constants.OPTIONS_DELETE)){ %>
										<button class="btn btn-danger btn-xs" onclick="removereg('<%= bean.getId() %>');"><i class="fa fa-trash-o "></i></button>
									<%}%>	
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PAISES, Constants.OPTIONS_VIEW)){ %>
