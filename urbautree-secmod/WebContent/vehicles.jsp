...
<html lang="en">
	...
	<%
		...
		ArrayList<VehicleBean> list = vm.getVehicles( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((VehicleBean)list.get( 0 )).getTotal_regs();
		}
	%>
	<script>
		function edit( id ){
			location.replace( "vehicle-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "vehicle-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "vehicle-detail.jsp?mode=view&id="+id);
		}
		function add(){
			location.replace( "vehicle-detail.jsp?mode=add" );
		}
	</script>
	</head>
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
                          ...
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> Vehiculos </h4>
	                  	  	  <hr>
	                  	  	  <thead>
                              <tr>
                                  <th class="hidden-phone">Marca</th>
                                  <th>Modleo</th>
                                  <th class="hidden-phone">No. Placa</th>
                                  <th>No. Chasis</th>
                                  <th>Estado</th>
                                  <th>Lic. Transportar Pirotecnia</th>
                                  <th>Lic. Vencimiento</th>
                              ...
                              <tbody>
                              <%
                              	for( VehicleBean us : list ){
                              %>
                              <tr>
                                  <td class="hidden-phone"><%= us.getMarca() %></td>
                                  <td><%= us.getModelo() %></td>
                                  <td class="hidden-phone"><%= us.getNoPlaca() %></td>
                                  <td><%= us.getNoChasis() %></td>
                                  <td><%= us.getEstado() %></td>
                                  <td><%= us.getLicPirotecnia() %></td>
                                  <td><%= us.getLicVencimiento() %></td>
