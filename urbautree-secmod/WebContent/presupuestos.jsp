...
<html lang="en">
	...
	<%		
		...
		ArrayList<PresupuestoBean> list = main.get( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((PresupuestoBean)list.get( 0 )).getTotal_regs();
		}
	...
	<script>
		function edit( id ){
			location.replace( "presupuestos-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "presupuestos-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "presupuestos-detail.jsp?mode=view&id="+id);
		}
		function add(){
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          ...
          <div class="col-lg-6">
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
						  ...
						  <% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PRESUPUESTO, Constants.OPTIONS_ADD)){ %>		
                          ...
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> PRESUPUESTOS</h4>
	                  	  	  <hr>
	                  	  	  <thead>
                              <tr>
                                  <th class="hidden-phone">Año</th>                                                                    
                                  <th class="hidden-phone">Mes</th>
                                  <th class="hidden-phone">Proyectado</th>
                                  <th class="hidden-phone">Ejecutado</th>                                  
                                  <th></th>
                              </tr>
                              </thead>
                              <tbody>
                              <%
                              	for( PresupuestoBean bean : list ){
                              %>
                              <tr>
							  	<td><%= bean.getAnio() %></td>
							  	<td><%= Constants.MESES[bean.getMes()]%></td>
							  	<td><%= bean.getProyectado()%></td>
							  	<td><%= bean.getEjecutado()%></td>
								<td>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PRESUPUESTO, Constants.OPTIONS_MODIFY)){ %>
										<button class="btn btn-primary btn-xs" onclick="edit('<%= bean.getId() %>');"><i class="fa fa-pencil"></i></button>
									<%}%>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_PRESUPUESTO , Constants.OPTIONS_DELETE)){ %>
		...
		%>
					  ...
					  <% if( backButton ) {%>
					  <li class="previous">
					    		<a href="rols.jsp?q=<%= request.getParameter("q") %>&from=<%= from - Constants.ITEMS_PER_PAGE  %>">
					  ...
					  <% } %>
					    <% if( forwardButton ){  %>
					    <li class="next">
					    	<a href="rols.jsp?q=<%= request.getParameter("q") %>&from=<%= end  %>">
