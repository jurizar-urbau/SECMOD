...
<html lang="en">
	...
	TiposRubrosMain main= new TiposRubrosMain();
		...
		ArrayList<TipoRubroBean> list = main.get( request.getParameter("q"), from );
		int total_regs = -1;
		
		if( list.size() > 0 ){
			total_regs = ((TipoRubroBean)list.get( 0 )).getTotal_regs();
		}
	...
	<script>
		function edit( id ){
			location.replace( "tiposrubros-detail.jsp?mode=edit&id="+id);
		}
		function removereg( id ){
			location.replace( "tiposrubros-detail.jsp?mode=remove&id="+id);
		}
		function view( id ){
			location.replace( "tiposrubros-detail.jsp?mode=view&id="+id);
		}
		function add(){
			location.replace( "tiposrubros-detail.jsp?mode=add" );
		}
	</script>
	</head>
          ...
          <div class="col-lg-6">
          	...
          	<div class="row mt">
          		...
          		<div class="content-panel">
						  ...
						  <% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_TIPOSRUBROS, Constants.OPTIONS_ADD)){ %>		
                          ...
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> TIPOS DE RUBROS</h4>
	                  	  	  <hr>
	                  	  	  <thead>
                              <tr>
                              ...
                              <tr>
								...
								<td>
									...
									<%}%>
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_TIPOSRUBROS, Constants.OPTIONS_DELETE)){ %>
										<button class="btn btn-danger btn-xs" onclick="removereg('<%= bean.getId() %>');"><i class="fa fa-trash-o "></i></button>
									<%}%>	
									<% if(Authorization.isAuthorizedOption(loggedUser.getRol(), Constants.NAME_TIPOSRUBROS, Constants.OPTIONS_VIEW)){ %>
